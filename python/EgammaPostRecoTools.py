...
###run regression
    if cfg.isMiniAOD:
        if cfg.runRegression:
            from RecoEgamma.EgammaTools.regressionModifier_cfi import *  
            #regression_modifier = regressionModifier80X
            #regression_modifier.vertexCollection = cms.InputTag("offlineSlimmedPrimaryVertices")
            regression_modifier = regressionModifier
            modifiers.append(regression_modifier)
###run regression
...
def _setupEgammaPostRecoSeq(*args,**kwargs):
    ...
    print ("SJ!!!! inside _setupEgammaPostRecoSeq, applyEnergyCorrections and applyVIDOnCorrectedEgamma are %d %d"%(cfg.applyEnergyCorrections, cfg.applyVIDOnCorrectedEgamma))
...
def setupEgammaPostRecoSeq(process,
    ...
    print ("SJ!!! applyEnergyCorrections is %d"%applyEnergyCorrections)
    _setupEgammaPostRecoSeq(process,applyEnergyCorrections=applyEnergyCorrections,applyVIDOnCorrectedEgamma=applyVIDOnCorrectedEgamma,era=era,runVID=runVID,runRegression=runRegression,runEnergyCorrections=runEnergyCorrections,applyEPCombBug=applyEPCombBug,isMiniAOD=isMiniAOD)
