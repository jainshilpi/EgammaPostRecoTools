...
def _setupEgammaEnergyCorrectionsAOD(process,eleSrc=cms.InputTag('gedGsfElectrons'),phoSrc=cms.InputTag('gedPhotons'),applyEnergyCorrections=False,era="2017-Nov17ReReco",runEnergyCorrections=True,applyEPCombBug=False):
...
def _setupEgammaPostRECOSequenceAOD(process,applyEnergyCorrections=False,applyVIDOnCorrectedEgamma=False,era="2017-Nov17ReReco",runVID=True,runEnergyCorrections=True,applyEPCombBug=False):
    ...
    if applyVIDOnCorrectedEgamma:
        raise RuntimeError('Error in postRecoEgammaTools, _setupEgammaPostRECOSequenceAOD can not currently apply VID on corrected E/gammas in AOD due to ValueMap issues'.format(applyEnergyCorrections,applyVIDOnCorrectedEgamma))
    if applyEnergyCorrections: 
        raise RuntimeError('Error in postRecoEgammaTools, _setupEgammaPostRECOSequenceAOD can not currently produce new collections with E/gamma energies corrected in AOD due to ValueMap issues'.format(applyEnergyCorrections,applyVIDOnCorrectedEgamma))
    ...
    _setupEgammaEnergyCorrectionsAOD(process,eleSrc=eleSrc,phoSrc=phoSrc,applyEnergyCorrections=applyEnergyCorrections,era=era,runEnergyCorrections=runEnergyCorrections,applyEPCombBug=applyEPCombBug)
